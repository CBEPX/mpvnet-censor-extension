...
- Future-schema намеренно не понижается автоматически, но теперь имеет явный
  ...
  поля удаляются, исходник остаётся в `.bak`.
- Compile reference получает отдельные SHA-256 для macOS arm64 и Windows x64.
  Маленькое file-based .NET-приложение читает lock file и считает хеш, поэтому
  отдельная зависимость от Node.js больше не нужна.
...
- Первый реальный runtime-smoke прошёл, но оставил `%LOCALAPPDATA%\CensorPlayer`
  ...
  отдельный временный `%LOCALAPPDATA%`; защитный отказ установщика не ослаблен.
