...
**Статус:** реализуется прямое создание интервалов без обязательного входного
файла и упрощение интерфейса для обычного сценария с 10–20 сценами. После
локальных проверок требуется exact-head Windows CI, полный review через Claude
Opus 5 и физическая проверка нового DLL. Полный portable/installer по-прежнему
нельзя публиковать до закрытия source-provenance gate.
...
- Постоянно показывать только F7/F8, удаление, undo/redo, переход к началу,
  явное применение и сохранение. Редкие операции, смещения, импорт, Save As и
  экспорт поместить под сворачиваемое `Дополнительно`.
