...
**Дата версии:** 19 августа 2026 года

> Основной сценарий — создание интервалов прямо во время просмотра, без заранее
> подготовленного файла. Пользователь также может явно запросить кандидаты у
> `timings.rte` и проверить их перед добавлением в черновик. SQLite, библиотека
> фильмов, долговременные привязки, fingerprint и миграции хранилища исключены.
...
8. позволяет выбрать корректную цель SDR/HDR и временно перейти на программное
   декодирование для проблемного Dolby Vision.
...
- логи и диагностика;
- явный поиск кандидатов на `timings.rte` и собственный GET-only агрегатор;
- режимы вывода `Авто / SDR / HDR` и совместимость Dolby Vision;
...
- локальный или фоновый каталог расписаний;
- поиск расписания по fingerprint;
- перенос привязок после переименования фильма;
- массовая индексация каталогов;
- облачная синхронизация;
- фоновые сетевые запросы и массовое скачивание расписаний;
- автоматическое применение таймкодов без предпросмотра;
...
flowchart LR
    ...
    R[timings.rte / свой агрегатор] -->|явный GET| E
...
- escaping;
- разбор свободного текста онлайн-таймингов и модель кандидатов;
...
### 4.3. `Censor.Timings.Api`

Необязательный ASP.NET Minimal API без базы данных:

- проксирует только GET-запросы к `timings.rte`;
- возвращает версионированную модель поиска и кандидатов;
- ограничивает частоту запросов и размер ответа;
- хранит ответы в памяти десять минут;
- поставляется отдельным Linux-контейнером.
...
- fingerprint фильма.
- историю онлайн-поиска и ответы внешнего сервиса.
...
### 6.6. Онлайн-кандидаты

1. Пользователь открывает `Найти онлайн…`, проверяет название и отдельно
   запускает поиск; до нажатия `Найти` сетевого запроса нет.
2. Фильм можно найти по названию. Вместо этого разрешено сразу указать ID
   Кинопоиска или официальную ссылку на фильм.
3. Ответ показывает источник, длительность, статус, голоса и исходный текст.
4. Полный интервал можно выбрать для импорта. Точка только перематывает плеер и
   отмечает начало; `Чисто` и неразобранный текст остаются справкой.
5. Импорт добавляет интервалы в текущий несохранённый черновик одной операцией
   undo. Файл создаётся только после отдельного `Сохранить файл`.

Запрос использует `SessionToken` и проверяет `media_session_id` после `await`.
Смена фильма отменяет запрос. Ошибка выбранного агрегатора не включает прямой
...
### 10.7. SDR, HDR и Dolby Vision

Расширение поддерживает три цели вывода:

```text
Авто — не менять параметры mpv.net; после SDR/HDR восстановить прежние
SDR  — hint=yes, mode=target, trc=gamma2.2, prim=bt.709
HDR  — hint=yes, mode=target, trc=pq, prim=bt.2020
```

Явные SDR/HDR работают только с `current-vo=gpu-next`. Расширение не меняет
`vo`, tone mapping, peak detection, ICC или `d3d11-output-csp`. После записи
параметров оно читает `video-target-params`; несовпадение или тайм-аут
приводят к откату.

Флажок совместимости Dolby Vision временно задаёт `hwdec=no`, выполняет
`video-reload` и проверяет `hwdec-current`. Значение не сохраняется и
восстанавливается при смене фильма. Эта настройка нужна для проблемного
Profile 5, но не заменяет физическую проверку конкретного GPU, драйвера и
монтажа.
...
- применить сохранённую цель `Авто / SDR / HDR` и проверить readback;
...
- восстановить исходный `hwdec`, если была включена совместимость Dolby Vision;
- перейти в `IDLE`.
...
- `current-vo`, цель вывода, `video-target-params`, `hwdec` и `hwdec-current`;
...
- онлайн-запрос выполняется только после действия пользователя, ограничен по
  времени и размеру и отменяется при смене фильма;
- в журнал не попадают строка поиска и исходный текст внешнего тайминга;
...
16. онлайн-ответ никогда не применяется без предпросмотра и не переносится в
    другую media session;
17. режимы SDR/HDR подтверждены через `video-target-params`, а ошибка приводит
    к откату;
18. совместимость Dolby Vision действует только до смены фильма.
...
> Windows P0 — это .NET extension для зафиксированной версии mpv.net без
> изменений. Пользователь создаёт интервалы вручную, импортирует файл или явно
> запрашивает кандидаты у `timings.rte`; онлайн-ответ всегда проходит
> предпросмотр. Расписание хранится только в текущей media session и при желании
> сохраняется как sidecar. SQLite, fingerprint и долговременные привязки не
> используются. Extension управляет статическим FFmpeg blur, аудиокомпрессией и
> проверяемой целью SDR/HDR через libmpv. Форк mpv.net создаётся только при
> доказанной технической необходимости.
