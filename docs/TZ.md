...
> В нормативной версии v1.3.2 основным сценарием стало создание интервалов прямо во время просмотра, без заранее подготовленного файла. SQLite, библиотека фильмов, долговременные привязки расписаний, fingerprint и миграции хранилища по-прежнему исключены. Расписание действует только в текущей сессии воспроизведения или автоматически загружается из sidecar-файла с тем же базовым именем, что и фильм.
...
1. позволяет отметить интервалы размытия прямо во время просмотра без заранее созданного файла;
...
- импорт SRT и WebVTT;
- прямое создание интервалов во время просмотра;
- необязательный импорт файла через file picker;
...
2. Открывает `Цензура → Интервалы…`.
3. Нажимает `Добавить вручную`. Новая строка получает текущую позицию и конец
   через секунду; если позиция временно недоступна, используются
   `00:00:00` и `00:00:01`. Пользователь исправляет начало, конец и при желании
   примечание прямо в строке. Обычно фильм содержит 10–20 таких сцен.
4. При желании использует F7/F8, чтобы быстрее отметить сцену во время
   просмотра.
5. Нажимает `Применить интервалы`: размытие начинает работать сразу, даже если
   файл ещё не сохранён.
6. Нажимает `Сохранить файл`. Первый диалог предлагает путь
   `<каталог фильма>\<имя фильма>.censor.txt`, но запись происходит только после
   подтверждения пользователя.
7. При `EndFile` активные фильтры очищаются. При следующем открытии фильма
   сохранённый sidecar может загрузиться автоматически.

Импорт `*.censor.txt`, SRT или WebVTT остаётся дополнительным сценарием в
разделе `Дополнительно`. Файл не привязывается к фильму в базе.
...
Если при переходе A → B остались несохранённые изменения для A, они не
применяются к B и показываются как отдельный несвязанный документ. Пользователь
может сохранить их для A, перенести на B с обновлением названия и длительности
либо удалить. Активный sidecar B продолжает работать во время выбора.
...
- при сохранении комментарии, стоявшие после интервалов, переносятся в header block перед интервалами.
...
Цензура
├── Интервалы…
├── Отметить начало интервала
├── Отметить конец интервала
├── Применить интервалы
└── Выключить размытие
```
...
```text
Интервалы | Настройки | Диагностика
```

Отдельной вкладки `Библиотека` нет.

### 8.3. Вкладка «Интервалы»

Над таблицей показываются имя фильма, количество интервалов, состояние
размытия и короткие признаки `Не применено` / `Не сохранено`. Полный путь,
технический статус filtergraph и подробности загрузки находятся только на
вкладке `Диагностика`. Предупреждения занимают место лишь тогда, когда они
есть.

Если интервалов нет, таблицу заменяет подсказка: `Интервалов пока нет. Нажмите
«Добавить вручную» и укажите время сцены.` Без открытого фильма команды F7/F8,
добавление и применение недоступны.
...
- постоянно видны `Добавить вручную`, `Удалить`, `Отменить`, `Повторить`,
  `Перейти к началу`, `Применить интервалы`, `Сохранить файл` и
  `Дополнительно`;
- новая строка выделяется, и редактор сразу открывает поле начала. Ввод
  `ЧЧ:ММ:СС` равнозначен `ЧЧ:ММ:СС.000`;
- под `Дополнительно` находятся F7/F8, импорт, дублирование, объединение,
  разделение, сдвиг границ на ±100 мс, навигация, общее смещение, сдвиг всех
  интервалов, `Сохранить как…`, экспорт SRT/VTT и удаление несохранённых
  изменений. F8 недоступна, пока начало не отмечено;
- объединяются только соприкасающиеся или пересекающиеся соседние строки;
  временной разрыв не заполняется автоматически;
- undo/redo хранит минимум 100 действий.

### 8.4. Authoring hotkeys
...
Первое сохранение предлагает рядом с локальным фильмом путь
`<basename>.censor.txt`. Пользователь всегда подтверждает путь в диалоге;
скрытой записи нет. Для URL, восстановленного или несвязанного документа
используются последняя папка и имя `schedule.censor.txt`.

После первого сохранения путь и хеш остаются в текущем session-state, поэтому
повторное `Сохранить файл` не открывает диалог. Основное сохранение всегда
создаёт канонический `.censor.txt`; SRT и WebVTT доступны только как отдельный
экспорт и не помечают изменения сохранёнными.
...
Новый документ получает название фильма и `media-duration-ms`, если mpv сообщил
длительность. Применение выполняется только явно и повторно проверяет, что
документ принадлежит текущему media session. Файл восстановления не содержит
путь к фильму и поэтому всегда открывается как несвязанный документ.
...
Неизвестная будущая schema не понижается автоматически. UI сообщает, почему
запись заблокирована, и предлагает отдельное подтверждаемое действие на вкладке
`Диагностика`: известные нормализованные значения записываются как schema 1,
неизвестные поля удаляются, исходный файл сохраняется в
`settings.json.pre-repair`, который не перезаписывается обычным сохранением.
...
### 10.4. Защитные ограничения

Обычный фильм ожидаемо содержит 10–20 интервалов размытия. Лимит 10 000 —
защита от чрезмерного входного файла, а не целевая нагрузка редактора.
Compiler всё равно должен:
...
- не более 10 000 исходных интервалов на входе;
...
При импорте SRT/VTT каждый cue превращается в blur interval, а его текст — в
note. Перед ручным импортом UI просит подтверждение, чтобы обычный файл
субтитров нельзя было случайно принять за расписание размытия.

### 13.2. Экспорт P0

- canonical `*.censor.txt` — полное сохранение расписания;
- SRT и WebVTT — экспорт копии только с интервалами и текстом;
- diagnostic JSON.

Перед экспортом SRT/WebVTT UI предупреждает, что название, смещение,
lead-in/lead-out и служебные строки будут потеряны. Такой экспорт не меняет
исходный путь, не помечает черновик сохранённым и не удаляет recovery-файл.
