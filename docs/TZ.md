...
3. На нужной сцене нажимает F7, а в её конце — F8. Обычно фильм содержит
   10–20 таких сцен.
4. При необходимости исправляет границы в таблице.
...
Если интервалов нет, таблицу заменяет подсказка: `Интервалов пока нет. Во
время просмотра отметьте начало и конец сцены`. Без открытого фильма команды
F7/F8, добавление и применение недоступны.
...
- постоянно видны `Отметить начало (F7)`, `Отметить конец (F8)`, `Удалить`,
  `Отменить`, `Повторить`, `Перейти к началу`, `Применить интервалы`,
  `Сохранить файл` и `Дополнительно`;
- F8 недоступна, пока начало не отмечено; созданный интервал выделяется;
- под `Дополнительно` находятся импорт, ручное добавление, дублирование,
  объединение, разделение, сдвиг границ на ±100 мс, навигация, общее смещение,
  сдвиг всех интервалов, `Сохранить как…`, экспорт SRT/VTT и удаление
