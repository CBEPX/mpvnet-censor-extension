...
> Нормативная версия v1.3.1 включает результаты архитектурного review. Из архитектуры удалены SQLite, библиотека фильмов, долговременные привязки расписаний, fingerprint и миграции хранилища. Расписание применяется только к текущей сессии воспроизведения либо автоматически загружается как sidecar-файл с тем же базовым именем, что и фильм.
...
Появление необработанной заставки при открытии фильма не является блокером, если эта заставка не попадает в заданный интервал размытия.
...
Персистентно сохраняются только:
...
Extension API должен быть использован для:

- получения событий текущего файла;
- чтения свойств libmpv;
- выполнения команд;
- отображения отдельного окна;
- интеграции с контекстным меню и горячими клавишами.
...
First-frame leak сам по себе не является основанием для форка.
...
5. Extension парсит файл и показывает краткое резюме:
   ...
   - warnings;
   - расхождение длительности, если metadata присутствует.
...
Выбранный файл не привязывается к фильму в базе и не подхватывается при следующем открытии, если он не является sidecar.
...
Drop на главное окно mpv.net не считается обязательным для P0, потому что mpv.net может трактовать текстовый файл как новый media input.

### 6.3. Sidecar без базы

Если включено `autoLoadSidecar`, extension проверяет рядом с фильмом точные имена:
...
- для B выполняется только sidecar lookup;
- при отсутствии sidecar статус становится `NO SCHEDULE`.

### 6.5. Повторное открытие фильма

Повторное открытие не считается основным сценарием.
...
- после `|` располагается необязательная заметка;
...
- повтор известного metadata key является ошибкой;
...
- разница до `durationToleranceMs` считается допустимой;
- большая разница показывает blocking warning перед применением;
...
Имя фильма и размер файла не используются как криптографическая привязка и служат только информационными подсказками.
...
```text
Цензура
├── Статус: ACTIVE / NO SCHEDULE / ERROR
...
Связь в базе не создается. При сохранении рядом с фильмом будущий автоподхват обеспечивается только соглашением об имени.
...
```text
<MPVNET_HOME>\extensions\CensorExtension\settings.json
```
...
Extension не переключает blur таймером.
...
Expression используется в `enable` video filter. Решение принимается по timestamp каждого кадра внутри video pipeline.

### 10.2. Концептуальный filter

```text
@censor_blur_000:lavfi=[gblur=sigma=30:steps=2:enable='EXPRESSION']
```
...
Следует использовать typed/native mpv commands и зарезервированные labels с prefix:
...
- seek назад и вперед немедленно дает правильный blur;
...
Редактирование является режимом UI, а не состоянием playback lifecycle.
...
- результат asynchronous parse применяется только при совпадении session ID;
...
Любой callback extension должен быть защищен общим exception boundary:
...
При импорте SRT/VTT каждый cue становится blur interval; текст cue становится note.
...
Особое внимание scripts, которые:
...
- playback без активного blur не получает заметного снижения FPS;
- активный blur проходит тесты 1080p30, 1080p60 и 4K30;
...
- удаление schedule не является функцией P0;
- сетевые запросы отсутствуют;
- auto-update отсутствует.

SQL и защита ZIP import не требуются, потому что базы и библиотечных архивов нет.
...
- затем реализация;
- изменение schema или runtime contract сопровождается документацией;
...
Обязательные направления review:
...
- ошибки содержат понятное действие для пользователя;
...
> Windows P0 реализуется как .NET extension для неизмененной зафиксированной версии mpv.net. Пользователь вручную загружает текстовый файл с таймкодами через меню или drag-and-drop; дополнительно поддерживается автоматический exact-sidecar `<film>.censor.txt`. Расписание хранится только в памяти текущей media session и очищается при смене фильма. SQLite, fingerprint, каталог фильмов и долговременные привязки не используются. Extension компилирует интервалы в статический FFmpeg timeline filter и применяет его через libmpv. Форк mpv.net создается только при доказанной технической необходимости.
